...
function renderTickets() {
  ...
  ticketsToDisplay.forEach((ticket) => {
    ...
    row.innerHTML = `
      ...
      <td style="color:#315db7; font-weight:700;">
        ${esc(category)}
      </td>
      ...
      <td>
        <a
          class="btn-outline"
          href="/details?id=${encodeURIComponent(id)}"
        >
          View
        </a>
      </td>
...
function applyFilters() {
  ...
  filteredTickets = tickets.filter((ticket) => {
    const ticketCategory = getCategory(ticket);
    const ticketPriority = getPriority(ticket);
    const ticketStatus = getStatus(ticket);

    const categoryMatches = category === "All" || ticketCategory === category;

    const priorityMatches = priority === "All" || ticketPriority === priority;

    const statusMatches = status === "All" || ticketStatus === status;
